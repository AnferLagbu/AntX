...
- 64 位长模式启动，4GB 恒等映射页表
- 完整的 IDT/PIC 中断框架，32 条异常 + 16 条 IRQ + syscall
- **lwIP 2.2.1** 协议栈 — DHCP 自动获取 IP，HTTP 服务器，DNS 解析，Ping
- E1000 千兆网卡驱动，Rust 实现
- **HVFS** — 自研文件系统，类 ext2 设计，三级间接块，FSCK，磁盘持久化
- **PWID** — 基于能力的权限模型，令牌委托、信任链、域隔离
- MLFQ 多级反馈调度器 + RT 优先级队列
- VFS 挂载表、文件描述符表、Barrier 故障恢复
- 自举日志系统 (KLog)，串口输出，格式化宏
