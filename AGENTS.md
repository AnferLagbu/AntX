...
项目使用 Gitee 作为唯一远程仓库，remote 名为 `origin`. 见 §11.4.
...
遇到方案 A vs B 选择时，AI 应停下询问用户而非自行选择（属"决策者"职责）。**绝不盲从 Linux 实现**（§11.1 强调）。
...
| 不读 AGENTS.md §11 AI 行为准则 | 必读！LLM 行为准则在此 |
