...
- **REVIEW-FINDING-001** — `services/proc/fd_alloc.rs:264,331` `.unwrap()` → **已修复: 替换为 `?` 操作符 (2026-07-04)**.
- **REVIEW-FINDING-002** — `framework` 内 `println!` → **`#[cfg(test)]` 模块内, test 环境有 std, 安全**.
- **REVIEW-FINDING-003** — `AGENTS.md` 引用 `ci/test_qemu.sh` → **已修正为 `./scripts/qemu_boot_test.sh` (2026-07-04)**.
- **REVIEW-FINDING-004** — `AGENTS.md` 引用 `rustfmt.toml` → **已创建 `rustfmt.toml` (2026-07-04)**.
- **REVIEW-FINDING-005** — clippy CI → **已在 AGENTS.md §2.4 加注"当前 CI 仅强制 unsafe_code lint" (2026-07-04)**.
- **REVIEW-FINDING-006** — framework `pub mod` 49 个 → **架构观察项, 审计脚本已提供防御, 无需代码变更**.
- **REVIEW-FINDING-007** — `pub unsafe fn` 非 arch 9 处 → **评估项, 详见 014 arch unsafe 审计结论**.
...
- **REVIEW-FINDING-010** — dead_code 注释 → **已为 6 处无注释的 allow(dead_code) 补充说明 (net/init.rs 5处, boot/mod.rs 1处)**.
- **REVIEW-FINDING-019~023** — softirq/workqueue → **已注册 Timer/NetRx/NetTx/Block/Tasklet softirq handler + 实现 tasklet 框架 (register_tasklet/schedule_tasklet) + workqueue 占位**.
