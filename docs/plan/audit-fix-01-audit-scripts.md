...
- **B01-01. 审计工具链门禁失效**
  ...
  - 状态：[X] (2026-08-19 23 条工程任务全部完成, 详见底部"实施总体报告")
...
- **B01-02. 验证门槛依赖失效脚本**
  ...
  - 状态：[X] (2026-08-19 审计脚本修复完成, 验证门槛恢复可信)
...
- **B01-03. services_boundary.py 退出码纳入 HIGH/MEDIUM（META-P0-01）**
  ...
  - 状态：[X] (commit 4ba454ab, HIGH 违规阻断 exit 1)
...
- **B01-04. services_boundary.py 黑名单补全 + allow-list 落地（META-P0-02）**
  ...
  - 状态：[X] (commit 4ba454ab, 黑名单补全 22 项, 识别 12 处 HIGH 违规)
...
- **B01-05. services_boundary.py 匹配 pub use（META-P0-03）**
  ...
  - 状态：[X] (commit 4ba454ab, 正则支持 pub use / pub(crate) use)
...
- **B01-07. block_registration.py 检测函数名修正（META-P0-05）**
  ...
  - 状态：[X] (commit c08f3eb6, PATTERN 修正 + 负测试 PASS)
...
- **B01-08. once_cell.py 正则绕过修复（META-P0-06）**
  ...
  - 状态：[X] (commit 9d1ad45b, 正则支持 pub use / OnceCell)
...
- **B01-09. comment_language.py 行尾注释检测（META-P0-07）**
  ...
  - 状态：[X] (commit bda443f8, find_line_comment 状态机 + 位字段豁免)
...
- **B01-10. audit_unsafe.sh 废弃（META-P0-08）**
  ...
  - 状态：[X] (commit 3c9b79f2, 标记 DEPRECATED + 转发到 Python 版)
...
- **B01-11. public_api_docs.py 接入 CI（META-P0-09）**
  ...
  - 状态：[X] (commit 3f5ae2de, 缺陷修复 6 项, 1191 误报 → 1921 真缺漏)
...
- **B01-12. tcb_ratio.py 退出码恢复（P0-01）**
  ...
  - 状态：[X] (commit 1ce941e3 + f91f940e, --soft/--enforce 切换 + smoltcp 路径修正)
...
- **B01-13. safety_coverage.py 全量扫描（P0-02）**
  ...
  - 状态：[X] (commit 12a0cbf9, 动态发现 54 模块 343 文件, 2471 unsafe 引用)
...
- **B01-14. ci_check_services_unsafe.py vendored 排除（P0-04）**
  ...
  - 状态：[X] (commit 12a9ee21, 排除 97 vendored 文件, 0 违规)
...
- **B01-15. audit_unsafe.py SAFETY 窗口修复（META-P1）**
  ...
  - 状态：[X] (commit 3e4bfcae + d8ace27f, 60 行窗口 + 严格匹配 + Send/Sync 豁免; 漏报 2548 → 127)
...
- **B01-16. ci/audit.sh 门禁补漏（META-P1）**
  ...
  - 状态：[X] (commit b6e1337d + 1b4e9fd5, 3 处门禁 + ci/audit.sh 接受基线 ≤127)
...
- **B01-17. static_mut/repr_c/volatile fail-open 改 fail-closed（META-P1）**
  ...
  - 状态：[X] (commit 5dfdaccd, DECISION-047 fail-closed 原则统一应用)
...
- **B01-18. coupling/invariants 接入 CI（META-P1 + CI 缺口）**
  ...
  - 状态：[X] (commit 4c03a5ed, ci-lint.yml 新增 2 个 job: audit-coupling + audit-invariants)
...
- **B01-19. unwired_pub_fn.py 深度状态重置（META-P1）**
  ...
  - 状态：[X] (commit a58e3649, impl 深度重置 + rg 降级 Python 路径)
...
- **B01-20. auto_*.py 硬编码绝对路径（P0-06）**
  ...
  - 状态：[X] (commit 41c0bed3, 3 个工具统一改用 argv 推导)
...
- **B01-21. invariants.py I2 误报 safe 解引用（META-P1）**
  ...
  - 状态：[X] (commit c00e9e55, _scan_framework + _scan_services 排除 smoltcp)
...
- **B01-22. edition2024.py 正则缺陷（META-P1）**
  ...
  - 状态：[X] (commit f96d3aae, safe-cast/deref 排除; 10591 → 29 真实候选)
...
- **B01-23. implicit_deps.py 子串匹配误报（META-P1）**
  ...
  - 状态：[X] (commit ad18153f, 词边界 + 动态发现 + 关键字过滤; 2553 → 89)
...
- **B01-25. 全量回归**
  ...
  - 状态：[X] (commit ff8bff34, bash -n ci/audit.sh + tools/audit_unsafe.sh PASS, 24 个 commit 历史)
...
- **DECISION-047**
  ...
  - 状态：[X] (2026-08-19 已应用到 audit_static_mut/repr_c/volatile_access 三脚本, commit 5dfdaccd)
...
- **前置依赖（阻塞）**
  ...
  - 状态：[X] (2026-08-19 分册 01 全部完成, 后续分册 02-09 可启动)
