...
| E6 VFS 策略提取 | 进行中 (2/9) | 2026-06-10 | E6-1 flock 策略提取: services/fs/flock.rs 完整实现 (730+ 行), framework re-export; E6-2 inotify 策略提取: services/fs/inotify.rs 完整实现 (590+ 行), framework re-export + sys_inotify_read 保留 unsafe 用户缓冲区写入; 新增 inotify_read_events safe API; TCB 累计缩减 1,240 行; E6-7 DevFS 迁移 (295 行, 0 unsafe, 已有 SafeDevFs 代理); E6-8 ProcFS 迁移 (245 行, 0 unsafe, 已有 SafeProcFs 代理); E6-9 Chitin↔DevFS 联合与硬编码消除 (三阶段: 去硬编码→Chitin 桥接→接入 VFS); InitRamFS 不迁移 (启动时 cpio 解析器, 是 VFS 调用者而非被分发对象); 详见 [vfs-policy-extraction.md](vfs-policy-extraction.md) | 双架构 0w0e + 审计通过 |
