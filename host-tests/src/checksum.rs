...
mod tests {
    ...
    fn fletcher2_single_byte() {
        ...
        assert_ne!(ck.value[0], 0, "single byte should produce non-zero checksum");
    }
    ...
    fn fletcher2_different_lengths_same_prefix() {
        ...
        assert_ne!(ck1.value, ck2.value, "different lengths should produce different checksums");
    }
    ...
    fn checksum_kind_off_verify_always_true() {
        let ck = Checksum::compute(CksumKind::Off, b"any data");
        assert!(ck.verify(b"different data"), "Off checksum should verify any data");
    }
    ...
    fn sha256_checksum_short_data() {
        ...
        assert_eq!(ck1.value, ck2.value, "SHA256 checksum should be deterministic for short data");
    }
    ...
    fn sha256_checksum_known_vector() {
        ...
        assert_eq!(ck.value, expected, "SHA256('abc') should match FIPS 180-4 test vector");
    }
    ...
    fn sha256_checksum_empty() {
        ...
        assert_eq!(ck.value, expected, "SHA256('') should match FIPS 180-4 test vector");
    }
    ...
    fn sha256_checksum_long_data() {
        ...
        let ck1 = Checksum::compute(CksumKind::SHA256, data);
        let ck2 = Checksum::compute(CksumKind::SHA256, data);
        assert_eq!(ck1.value, ck2.value, "SHA256 should be deterministic for long data");
    }
}
