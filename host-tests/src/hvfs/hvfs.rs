...
impl HvfsData {
    ...
    fn setup_zil_datasets(&self) {
        ...
        }
        {
            ...
            if !ub_copy.root_bp.is_null() && self.deserialize_dataset_metadata(&ub_copy.root_bp) {
                has_persisted = true;
            }
            if !has_persisted {
                datasets[0].init(0);
    ...
    fn probe_partition_size(&self, part_start: u32) -> u64 {
        ...
        if unsafe { ata_disk_present(self.disk_drive) } == 0 {
            return 0;
        }
    ...
    pub fn open(&self, path: &str, flags: u32, pwid: u64) -> Result<i32, KernelError> {
        ...
        if !self.check_permission(&obj, pwid, 0x01) {
    ...
    pub fn write(&self, fd: u32, buf: &[u8], count: u32) -> i32 {
        ...
        if !self.check_permission(&obj, pwid, 0x02) {
            return -3;
        }
        if flags & 0x0400 != 0 {
            offset = obj.size;
        ...
        if self.is_disk_mode() && self.spa.write_bp(&new_bp, write_data) != 0 {
    ...
    fn serialize_dataset_metadata(&self, txg: u64) -> Option<HvBlockPointer> {
        ...
        let mut off = 32usize;
        for obj in &objects {
            Self::write_le64(&mut buf, off, obj.obj_id);
            off += 8;
            buf[off] = obj.obj_type as u8;
            off += 1;
            Self::write_le32(&mut buf, off, obj.block_size);
            off += 4;
            Self::write_le64(&mut buf, off, obj.nblocks);
            off += 8;
            Self::write_le64(&mut buf, off, obj.size);
            off += 8;
            let bp_bytes = unsafe {
            ...
            off += BP_BYTES;
            Self::write_le64(&mut buf, off, obj.atime);
            off += 8;
            Self::write_le64(&mut buf, off, obj.mtime);
            off += 8;
            Self::write_le64(&mut buf, off, obj.ctime);
            off += 8;
            Self::write_le64(&mut buf, off, obj.owner_pwid);
            off += 8;
            Self::write_le64(&mut buf, off, obj.group_pwid);
            off += 8;
            buf[off] = obj.sensitivity;
            off += 1;
            Self::write_le16(&mut buf, off, obj.pwid_perm);
            off += 2;
            Self::write_le32(&mut buf, off, obj.link_count);
            off += 4;
            Self::write_le32(&mut buf, off, obj.flags);
            off += 4;
            Self::write_le64(&mut buf, off, obj.birth_txg);
            off += 8;
        ...
        for (name, _value) in &dir_entries {
            ...
            Self::write_le16(&mut buf, off, name_bytes.len() as u16);
            off += 2;
    ...
    fn deserialize_dataset_metadata(&self, bp: &HvBlockPointer) -> bool {
        ...
        let next_obj_id = u64::from_le_bytes([
            ...
            for _ in 0..obj_count {
                ...
                let bp_slice = unsafe {
                    core::slice::from_raw_parts_mut(
                        &mut bp as *mut HvBlockPointer as *mut u8,
                        BP_BYTES,
                    )
                };
                ...
                off += 8;
                let owner_pwid = u64::from_le_bytes([
                    buf[off],
                    buf[off + 1],
                    buf[off + 2],
                    buf[off + 3],
                ...
                off += 8;
                let group_pwid = u64::from_le_bytes([
                    buf[off],
                    buf[off + 1],
                    buf[off + 2],
                    buf[off + 3],
                ...
                off += 1;
                let pwid_perm = u16::from_le_bytes([buf[off], buf[off + 1]]);
                off += 2;
                let link_count =
                    u32::from_le_bytes([buf[off], buf[off + 1], buf[off + 2], buf[off + 3]]);
                off += 4;
