...
static int test_pmm_alloc_multiple(void) {
    ...
    TEST_ASSERT_EQ(free_after_alloc, free_before - count);
...
static int test_pmm_alloc_consecutive(void) {
    ...
    uint64_t free_after = pmm_get_free_pages();
    TEST_ASSERT_EQ(free_after, free_before - count);
...
static int test_pmm_boundary(void) {
    ...
    TEST_ASSERT_EQ(total, free + used);
...
void test_pmm_register(void) {
    ...
    test_register_case(mod, "Stress test (100 allocations)", test_pmm_stress);
