...
use crate::kernel::fs::hvfs::hvfs::{get_hvfs, HVFS_O_CREAT, HVFS_O_WRONLY, HVFS_CAP_READ, HVFS_DISK_OK, HVFS_DISK_NO_DISK, 
...
impl DiskFsData {
    ...
    pub fn read(&mut self, system_fd: u32, buf: &mut [u8], count: u32) -> i32 {
        ...
        if !hvfs.check_permission(inode, pwid, crate::kernel::fs::hvfs::hvfs::HVFS_CAP_READ) {
            return -1;
        }
    ...
    pub fn write(&mut self, system_fd: u32, buf: &[u8], count: u32) -> i32 {
        ...
        if hvfs.write_file_data(inode_num, self.fds[idx].offset, &buf[..bytes_written as usize]) {
            if let Some(inode) = hvfs.get_inode_mut(inode_num) {
                ...
                inode.mtime = crate::kernel::fs::hvfs::hvfs::HvFsData::get_time();
                inode.dirty = true;
            }
        } else {
    ...
    pub fn truncate(&mut self, inode_num: u32, new_size: u64, pwid: u64) -> i32 {
        ...
        if !hvfs.check_permission(inode, pwid, crate::kernel::fs::hvfs::hvfs::HVFS_CAP_WRITE) {
            return -1;
        }
