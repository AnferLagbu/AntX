...
// 写入此 bit 前必须确保:
//
// 1. 像素时钟 PLL 已锁定 (DISPLAY-2.3a 完成)
// 2. 时序参数已配置 (DISPLAY-2.3b 完成)
// 3. 同步极性已配置 (本实装)
// 否则显示器可能收到无效信号, 显示异常.
//
// 厂商差异:
...
// - 通用 SoC: 通常单 bit TMDS enable

/// HDMI 同步极性寄存器偏移 (8-bit)。
const HDMI_SYNC_POL_REG_OFFSET: usize = 0x078;
/// 同步极性寄存器 H bit (bit 0)
const HDMI_SYNC_POL_H_BIT: u8 = 0x01;
/// 同步极性寄存器 V bit (bit 1)
const HDMI_SYNC_POL_V_BIT: u8 = 0x02;

/// HDMI TMDS 输出使能寄存器偏移 (8-bit)。
const HDMI_TMDS_ENABLE_REG_OFFSET: usize = 0x079;
/// TMDS 输出使能 bit (bit 0)
const HDMI_TMDS_ENABLE_BIT: u8 = 0x01;
...
unsafe fn configure_hdmi_sync_polarity(
    iomem: &IoMem,
    hsync_positive: bool,
    vsync_positive: bool,
) {
...
unsafe fn enable_hdmi_tmds_output(iomem: &IoMem) {
...
unsafe fn disable_hdmi_tmds_output(iomem: &IoMem) {
...
impl HdmiController {
    ...
    pub fn set_video_mode(&mut self, mode: VideoMode) -> Result<()> {
        ...
        //
        // 同步极性: 写入 1 字节 (bit 0=H, bit 1=V); 0=negative, 1=positive.
        // TMDS enable: 写入 1 字节 (bit 0=enable); 必须在 1+2 步之后调用.
        if let Some(iomem) = &self.iomem {
            // SAFETY: 同步极性寄存器 offset = 0x078 (1 字节) + TMDS enable 0x079 (1 字节),
            // 需 0x07A <= iomem.len(); 调用方通过 new_with_iomem* 构造时需保证 IoMem
            // 范围 >= 0x07A.
            unsafe {
                configure_hdmi_sync_polarity(
                    iomem,
                    mode.flags.hsync_positive,
                    mode.flags.vsync_positive,
                );
                enable_hdmi_tmds_output(iomem);
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_video_mode_flags_default_negative_sync() {
        // 默认 sync 极性必须 = false (negative, 现代显示器常用).
        let flags = VideoModeFlags::default();
        assert!(!flags.hsync_positive);
        assert!(!flags.vsync_positive);
        assert!(!flags.interlaced);
    ...
    #[test]
    fn test_video_mode_flags_positive_sync() {
        // 老式 CEA 模式 (e.g. 480i) 需要 positive sync.
        let flags = VideoModeFlags {
            interlaced: true,
            double_scan: false,
            hsync_positive: true,
            vsync_positive: true,
