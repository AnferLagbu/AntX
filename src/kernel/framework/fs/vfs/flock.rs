//! 文件锁 (flock + POSIX record locks)
//!
//! ## 动机
//!
//! 多进程并发访问同一文件需要协调机制:
//! - **flock**: BSD 风格整文件锁, 简单高效, 关联到 fd
//! - **POSIX record locks**: fcntl 字节范围锁, 精细控制, 关联到 (pid, inode, range)
//!
//! ## 架构
//!
//! ```text
...
impl PosixLockTable {
    ...
    /// 检查两个范围是否重叠
    ///
    /// 范围 A: [a_start, a_start + a_len)
    /// 范围 B: [b_start, b_start + b_len)
    /// len = 0 表示到文件末尾 (视为无穷大)
    ...
    /// 查找与指定范围冲突的 POSIX 锁
    ///
    /// 返回冲突锁的 (owner_pid, lock_type), 无冲突返回 None
    fn find_conflict(
        &self,
        ino: u32,
        pid: u32,
...
/// # 参数
/// - `fd`: 文件描述符
/// - `operation`: LOCK_SH | LOCK_EX | LOCK_UN, 可或 LOCK_NB
/// - `pid`: 调用进程 PID
/// - `ino`: fd 对应的 inode 号
    ...
    match lock_type {
        ...
        F_RDLCK | F_WRLCK => {
            ...
            if let Some((conflict_pid, conflict_type)) = table.find_conflict(ino, pid, start, len, lock_type) {
                // v1: 不实现阻塞等待, 即使 F_SETLKW 也返回冲突信息
                return Ok(Some(PosixLockConflict {
                    pid: conflict_pid,
                    lock_type: conflict_type,
                    start: 0,
