...
mod tests {
    ...
    fn test_dcache_miss() {
        ...
        assert!(dcache.lookup(1, "nonexistent").is_none());
    }

    #[test]
    fn test_dcache_negative() {
        let mut dcache = DCache::new();
        dcache.insert(1, "missing", NEGATIVE_INO, 0);
        assert!(matches!(
            dcache.lookup(1, "missing"),
            Some((NEGATIVE_INO, _))
        ));
    }
    ...
    fn test_dcache_invalidate_parent() {
        let mut dcache = DCache::new();
        dcache.insert(1, "bin", 10, 1);
        dcache.insert(1, "usr", 20, 1);
        dcache.insert(2, "home", 30, 1); // 不同父目录

        dcache.invalidate_parent(1);
        assert!(dcache.lookup(1, "bin").is_none());
        assert!(dcache.lookup(1, "usr").is_none());
        assert!(matches!(dcache.lookup(2, "home"), Some((30, 1)))); // 不受影响
    }
    ...
    fn test_dcache_invalidate_entry() {
        ...
        dcache.invalidate_entry(1, "bin");
        assert!(dcache.lookup(1, "bin").is_none());
        assert!(matches!(dcache.lookup(1, "usr"), Some((20, 1))));
    }
    ...
    fn test_dcache_update() {
        ...
        dcache.insert(1, "file", 20, 0); // 更新
        assert!(matches!(dcache.lookup(1, "file"), Some((20, 0))));
    }
    ...
    #[test]
    fn test_icache_update() {
        let mut icache = ICache::new();
        icache.insert(10, 1, 0o755, 4096, 1000);
        icache.insert(10, 1, 0o644, 8192, 2000); // 更新
    ...
    fn test_hash_distribution() {
        // 验证不同 key 产生不同哈希值
        let h1 = DCache::hash_key(1, "bin");
        let h2 = DCache::hash_key(1, "usr");
        let h3 = DCache::hash_key(2, "bin");
        assert_ne!(h1, h2);
        assert_ne!(h1, h3);
        assert_ne!(h2, h3);
    }
