//! NUMA (Non-Uniform Memory Access) — 拓扑感知与内存策略
//!
//! ## 设计
//!
//! 提供 NUMA 拓扑发现、内存策略管理和调度亲和性集成.
//!
//! ### 核心概念
//!
//! - **NumaNode**: 一个 NUMA 节点, 包含 CPU 集合 + 本地内存范围 + 距离表
//! - **NumaTopology**: 全局拓扑管理器, 维护节点列表和距离矩阵
//! - **NumaPolicy**: 进程/内存的 NUMA 策略 (default/bind/interleave/preferred)
//!
//! ### 与 Linux 的差异
//!
//! 1. 拓扑来源: QEMU `-numa` 参数通过 ACPI SRAT/SLIT 表传递,
//!    当前先用硬编码单节点拓扑, 后续接入 ACPI 解析
//! 2. 距离矩阵: 固定 [MAX_NUMA_NODES]^2, 不支持动态扩展
//! 3. 内存策略: 仅实现 default/bind/interleave 三种, preferred 暂等同于 bind
//!
//! ## SAFETY
//!
...
impl NumaMempolicy {
    ...
    /// 根据策略选择分配节点
    ///
    /// 返回目标 NUMA 节点 ID, 或 None 表示使用默认行为
...
pub struct NumaNode {
    /// 节点 ID
    pub id: u32,
    /// 本节点的 CPU 列表 (APIC ID 或逻辑 CPU ID)
    pub cpus: Vec<u32>,
    /// 本地内存起始物理地址
...
impl NumaTopology {
    ...
    /// 用单节点拓扑初始化 (UMA 回退)
    ///
    /// 当没有 SRAT/SLIT 表时, 假设所有 CPU 和内存在 node 0
...
pub fn sys_get_mempolicy(_mode_ptr: u64, _nodemask_ptr: u64) -> i64 {
    ...
    match result {
        Some((mode, mask)) => {
            // 写回用户空间 (暂用返回值传递, 后续接入 copy_to_user)
            let _ = (mode, mask);
            0
        }
        None => -(22i64), // EINVAL
...
///
/// `a0`: 策略模式 (0=default, 1=bind, 2=interleave, 3=preferred)
/// `a1`: nodemask (u64)
///
/// 返回: 0 成功, -errno 失败
#[no_mangle]
pub fn sys_set_mempolicy(mode: u64, nodemask: u64) -> i64 {
    ...
    // 验证 nodemask: bind/interleave 必须有有效节点
