...
///
/// 已知限制:
/// - 无 COW: 父子进程共享物理页 (clone_user_page_table_cow 在 VMM lock 下 page fault 死锁)
/// - NamespaceSet::fork_from 在 PROCESS_TABLE 锁内崩溃, 使用 new_init
#[unsafe(no_mangle)]
pub fn sys_fork() -> Pid {
    let parent_pid = SCHEDULER.current().unwrap_or(0);
    ...
    // namespace (new_init 避免 fork_from 崩溃)
