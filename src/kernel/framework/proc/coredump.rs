...
pub fn do_coredump(pid: u32, sig: u8, frame: u64) -> bool {
    ...
    if offset < note_end {
        ...
        while zoff < pad {
            ...
            crate::kernel::framework::fs::vfs::api::vfs_write(fd, zeros.as_ptr(), n as u32);
            zoff += n;
        }
        offset = note_end;
    }
    ...
    // 11. 关闭文件
    crate::kernel::framework::fs::vfs::api::vfs_close(fd);
...
fn fill_regs_from_frame(prstatus: &mut PrStatus, frame_addr: u64) {
    ...
    let frame = unsafe { &*(frame_addr as *const crate::kernel::framework::idt::types::InterruptFrame) };
