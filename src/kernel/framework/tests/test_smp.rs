...
fn test_per_cpu_sched_init() -> TestResult {
    use crate::kernel::framework::proc_tcb_legacy::scheduler::SCHEDULER_READY;
    check!(
...
fn test_per_cpu_current_valid() -> TestResult {
    use crate::kernel::framework::proc_tcb_legacy::scheduler::SCHEDULER;
...
fn test_user_proc_manager_destroy_no_kstack() -> TestResult {
    use crate::kernel::framework::proc_tcb_legacy::user_proc::USER_PROC_MANAGER;

    let pid = crate::kernel::framework::proc_tcb_legacy::scheduler::SCHEDULER
        .current()
        .unwrap_or(0);
    if pid == 0 {
