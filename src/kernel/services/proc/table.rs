//! 进程表 CRUD 安全代理
//!
//! 把 `kernel::proc::process::ProcessTable` 的 `*mut Process` 裸指针接口
...
pub use crate::kernel::proc::scheduler::SchedPolicy;
...
impl ProcessHandle {
    /// 构造新句柄
    #[inline]
    pub const fn new(pid: proc::Pid) -> Self {
        Self { pid }
    }

    /// 获取 PID
    #[inline]
    pub const fn pid(self) -> proc::Pid {
        self.pid
    }
}
...
pub fn allocate_pid() -> TableResult<proc::Pid> {
    PROCESS_TABLE
        .allocate_pid()
...
pub fn try_inc_ref(pid: proc::Pid) -> TableResult<()> {
...
/// 减少引用计数, 归零时回收 PCB
pub fn dec_ref_and_maybe_free(pid: proc::Pid) {
...
/// ```
pub fn with<F, R>(pid: proc::Pid, f: F) -> Option<R>
where
    F: FnOnce(&proc::process::Process) -> R,
{
    PROCESS_TABLE.with_process(pid, f)
}

/// 在持有表锁的情况下, 对进程执行可变闭包
pub fn with_mut<F, R>(pid: proc::Pid, f: F) -> Option<R>
where
    F: FnOnce(&mut proc::process::Process) -> R,
{
...
/// 获取进程状态
pub fn get_state(pid: proc::Pid) -> Option<proc::ProcessState> {
    with(pid, |p| p.get_state())
}

/// 设置进程状态 (安全版本, 自动检查状态转换合法性)
pub fn set_state(pid: proc::Pid, state: proc::ProcessState) -> TableResult<()> {
...
/// 获取进程优先级
pub fn get_priority(pid: proc::Pid) -> Option<proc::ProcessPriority> {
    with(pid, |p| p.get_priority())
}

/// 设置进程优先级
pub fn set_priority(pid: proc::Pid, priority: proc::ProcessPriority) -> TableResult<()> {
    with_mut(pid, |p| p.set_priority(priority));
    Ok(())
}

/// 是否内核进程
pub fn is_kernel(pid: proc::Pid) -> Option<bool> {
    with(pid, |p| p.is_kernel())
}

/// 设置内核/用户标志
pub fn set_kernel(pid: proc::Pid, is_kernel: bool) -> TableResult<()> {
    with_mut(pid, |p| p.set_kernel(is_kernel));
    Ok(())
}

/// 获取调度策略
pub fn get_sched_policy(pid: proc::Pid) -> Option<SchedPolicy> {
    with(pid, |p| p.get_sched_policy())
}

/// 设置调度策略
pub fn set_sched_policy(pid: proc::Pid, policy: SchedPolicy) -> TableResult<()> {
    with_mut(pid, |p| p.set_sched_policy(policy));
    Ok(())
}

/// 获取 RT 优先级 (0-99)
pub fn get_rt_priority(pid: proc::Pid) -> Option<u8> {
    with(pid, |p| p.get_rt_priority())
}

/// 设置 RT 优先级
pub fn set_rt_priority(pid: proc::Pid, priority: u8) -> TableResult<()> {
    with_mut(pid, |p| p.set_rt_priority(priority));
    Ok(())
}

/// 获取进程 PMM (Per-Memory Mapping) 字节数
pub fn get_pwm(pid: proc::Pid) -> Option<u64> {
    with(pid, |p| p.get_pwm())
}

/// 设置 PMM
pub fn set_pwm(pid: proc::Pid, pwm: u64) -> TableResult<()> {
...
/// 设置待处理信号位
pub fn signal_set(pid: proc::Pid, sig: u32) -> TableResult<()> {
    with_mut(pid, |p| p.signal_pending_set(sig));
    Ok(())
}

/// 获取待处理信号位图
pub fn signal_get(pid: proc::Pid) -> Option<u64> {
    with(pid, |p| p.signal_pending_get())
}

/// 清除待处理信号位
pub fn signal_clear(pid: proc::Pid, mask: u64) -> TableResult<()> {
...
where
    F: FnMut(&proc::process::Process) -> bool,
{
    let mut count = 0u32;
...
pub fn remove_and_free(pid: proc::Pid) {
    PROCESS_TABLE.remove_and_free(pid);
}

/// 强制分配 PID (内核线程专用, 0/1/2 等保留 PID)
pub fn allocate_reserved_pid() -> TableResult<proc::Pid> {
