...
pub fn send(pid: proc::Pid, sig: Signal) -> SignalResult<()> {
    if sig == Signal::NONE {
...
/// 检查进程是否有信号待处理
pub fn pending(pid: proc::Pid) -> Option<u64> {
    crate::kernel::services::proc::table::signal_get(pid)
}

/// 清除进程的信号位
pub fn clear(pid: proc::Pid, mask: u64) -> SignalResult<()> {
