...
impl PhysicalMemoryManager {
    ...
    pub fn init_bitmap(&mut self, reserved_after_kernel: u64) {
        ...
        self.bitmap = match NonNull::new(bitmap_virt as *mut u32) {
            Some(ptr) => {
    ...
    /// ✅ P1-10 修复: Set a bit in the bitmap (mark page as used) - 原子操作
    /// 
    /// 使用 AtomicU32::fetch_or 保证原子性, 防止多核并发分配冲突
    ...
    /// ✅ P1-10 修复: Clear a bit in the bitmap (mark page as free) - 原子操作
    /// 
    /// 使用 AtomicU32::fetch_and 保证原子性, 防止并发释放冲突
