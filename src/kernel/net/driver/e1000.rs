...
const E1000_RXD_STAT_DD: u8 = 1 << 0;
const E1000_RXD_ERR_CE: u8 = 1 << 0;
const E1000_RXD_ERR_SE: u8 = 1 << 1;
const E1000_RXD_ERR_SEQ: u8 = 1 << 2;
const E1000_RXD_ERR_RXE: u8 = 1 << 3;
                    ...
                    if dev.irq != 0 && dev.irq != 255 {
                        ...
                        unsafe {
                            ...
                            if dev.irq < 8 {
                                ...
                                let master_mask = pic_inb(0x21);
                                pic_outb(0x21, master_mask & !(1u8 << 2));
                                let slave_mask = pic_inb(0xA1);
    ...
    match &mut *E1000_DEVICE.lock() {
        Some(ref mut dev) => {
            ...
            match dev.send_packet(packet) {
                ...
                Err(_) => {
                    klog_err!(Net, "e1000_send: failed");
                    -1
                }
            }
        }
