...
extern err_t virtio_net_send(struct netif *netif, struct pbuf *p);
...
void qx_netif_init_virtio(struct netif *netif, const uint8_t *mac) {
    netif->hwaddr_len = 6;
    int i;
    for (i = 0; i < 6; i++) netif->hwaddr[i] = mac[i];
    netif->mtu = 1500;
    ...
    netif->output = etharp_output;
    netif->output_ip6 = ethip6_output;
    netif->linkoutput = virtio_net_send;
    netif->name[0] = 'e';
    netif->name[1] = 'n';
}
