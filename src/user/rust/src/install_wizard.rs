//! AntX 安装向导 — 6 步交互式系统安装
//!
//! 此模块被 init 进程和独立的 install 二进制共用
...
static mut DISK_LIST: [syscall::UserDiskInfo; MAX_DISKS] = [
    syscall::UserDiskInfo { disk_id: 0, present: 0, total_sectors: 0, sectors: 0, model: [0; 64] },
    syscall::UserDiskInfo { disk_id: 0, present: 0, total_sectors: 0, sectors: 0, model: [0; 64] },
    syscall::UserDiskInfo { disk_id: 0, present: 0, total_sectors: 0, sectors: 0, model: [0; 64] },
    syscall::UserDiskInfo { disk_id: 0, present: 0, total_sectors: 0, sectors: 0, model: [0; 64] },
];
...
fn detect_disks() -> i32 {
    ...
    let count = syscall::disk_list(&mut disk_ids);
    if count <= 0 {
        println("  [ERROR] No disks detected!");
        return -1;
    }
    ...
    for i in 0..(count as usize) {
        ...
        syscall::disk_info(disk_ids[i] as u32, info);
...
fn select_disk() -> i32 {
    println("");
    println("Select a disk for installation:");
    ...
    println("");
    println("  [WARNING] ALL DATA ON THIS DISK WILL BE ERASED!");
    print("  Selected: ");
...
fn install_system_files() -> i32 {
    println("");
    println("--- Step 3: System File Installation ---");
    println("");
    ...
    for (src, dst, desc) in &copies {
        if file_copy(src, dst) {
            print("  [OK] "); print(desc); println(" installed");
        }
    }
    0
}
...
fn config_root_pwid() -> i32 {
    println("");
    println("--- Step 4: Administrator PWID Setup ---");
    println("");
    println("Creating the first administrator identity.");
    println("This identity will have full system access.");
    println("");

    loop {
        print("Enter root password (min 4 chars): ");
        let mut pw1 = [0u8; 64];
        let len1 = read_line(&mut pw1);

        if len1 < MIN_PASSWORD_LEN {
            print("Password too short! Minimum ");
            print_dec(MIN_PASSWORD_LEN as i64);
        ...
        println("");
        println("Creating root identity...");
        let mut p = [0u8; 65];
        p[..len1].copy_from_slice(&pw1[..len1]);
        p[len1] = 0;
        let r = syscall::auth_create_first(&p[..len1 + 1]);
        if r >= 0 {
            println("Root identity created successfully!");
            return 0;
        } else {
            print("Failed to create root identity (error: ");
            print_dec(r as i64);
...
fn config_system() {
    ...
    if fd >= 0 {
        syscall::fs_write(fd, &name_slice[..name_len]);
        syscall::fs_close(fd);
    }
    println("");
...
fn complete_page() -> i32 {
    println("");
    println("--- Step 6: Finalizing Installation ---");
    println("");
    ...
    syscall::fs_write(fd, b"installed\n");
    syscall::fs_close(fd);
    syscall::fs_sync();
    ...
    println("========================================");
    println("");
    println("AntX has been installed to your disk.");
...
pub fn run() {
    ...
    if format_disk() != 0 {
        println("Installation failed: Disk formatting error.");
        return;
    }
    if install_system_files() != 0 {
        println("Installation failed: File copy error.");
        return;
    }
...
pub fn check_needed() -> bool {
    ...
    if fd >= 0 { syscall::fs_close(fd); false } else { true }
}
