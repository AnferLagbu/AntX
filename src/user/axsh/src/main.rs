//! axsh — AntX Shell
//!
//! 19 条内建命令的交互式 Shell，通过 int 0x80 syscall 与内核交互。
...
fn cmd_help(_: &[*const u8], _: usize) {
...
fn cmd_cls(_: &[*const u8], _: usize) { print("\x1B[2J\x1B[H"); }

fn cmd_echo(args: &[*const u8], n: usize) {
    for i in 1..n { if i > 1 { print(" "); } print(as_str(args[i])); }
    println("");
}

fn cmd_exit(_: &[*const u8], _: usize) {
    println("Goodbye!"); RUNNING.store(false, core::sync::atomic::Ordering::Relaxed);
}

fn path_arg(args: &[*const u8], n: usize) -> Option<[u8; 256]> {
    if n < 2 { return None; }
    let s = as_str(args[1]).as_bytes();
...
fn cmd_fls(args: &[*const u8], n: usize) {
    let p = path_arg(args, n).unwrap_or_else(|| { let mut d = [0u8; 256]; d[0] = b'/'; d[1]=0; d });
    let fd = file_open(&p, O_RDONLY);
    if fd < 0 { print("fls: '"); print(core::str::from_utf8(&p).unwrap_or("?").trim_end_matches('\0')); println("' not found"); return; }
    let mut count = 0;
    loop {
...
fn cmd_fcd(args: &[*const u8], n: usize) {
    if let Some(p) = path_arg(args, n) {
        if env_chdir(&p) < 0 { print("fcd: '"); print(core::str::from_utf8(&p).unwrap_or("?").trim_end_matches('\0')); println("' not found"); }
    } else { println("fcd: missing path"); }
}

fn cmd_fpwd(_: &[*const u8], _: usize) {
    let mut cwd = [0u8; 128];
...
fn cmd_fcat(args: &[*const u8], n: usize) {
    if let Some(p) = path_arg(args, n) {
        let fd = file_open(&p, O_RDONLY);
        if fd < 0 { print("fcat: '"); print(core::str::from_utf8(&p).unwrap_or("?").trim_end_matches('\0')); println("' not found"); return; }
        let mut buf = [0u8; 512];
        loop {
            let n = fs_read(fd, &mut buf[..511]); if n <= 0 { break; }
            buf[n as usize] = 0;
...
fn cmd_fmk(args: &[*const u8], n: usize) {
    if let Some(p) = path_arg(args, n) {
        ...
        if fd < 0 { print("fmk: cannot create '"); print(core::str::from_utf8(&p).unwrap_or("?").trim_end_matches('\0')); println("'"); return; }
        fs_close(fd); print("Created: "); println(core::str::from_utf8(&p).unwrap_or("?").trim_end_matches('\0'));
    } else { println("fmk: missing file name"); }
}

fn cmd_fmd(args: &[*const u8], n: usize) {
    if let Some(p) = path_arg(args, n) {
        if fs_mkdir(&p) < 0 { print("fmd: cannot create '"); println(core::str::from_utf8(&p).unwrap_or("?").trim_end_matches('\0')); return; }
        print("Created: "); println(core::str::from_utf8(&p).unwrap_or("?").trim_end_matches('\0'));
    } else { println("fmd: missing directory name"); }
}

fn cmd_frm(args: &[*const u8], n: usize) {
    if let Some(p) = path_arg(args, n) {
        if fs_unlink(&p) < 0 { print("frm: cannot remove '"); println(core::str::from_utf8(&p).unwrap_or("?").trim_end_matches('\0')); return; }
        print("Removed: "); println(core::str::from_utf8(&p).unwrap_or("?").trim_end_matches('\0'));
    } else { println("frm: missing path"); }
}

fn cmd_fput(args: &[*const u8], n: usize) {
    if n < 3 { println("fput: usage: fput <file> <text>"); return; }
    let path = as_str(args[1]); let text = as_str(args[2]);
    ...
    let n = fs_write(fd, text.as_bytes()); fs_close(fd);
    print("Wrote "); print_dec(n as i64); println(" bytes");
}

fn cmd_fsync(_: &[*const u8], _: usize) { fs_sync(); println("Synced"); }

fn cmd_ilogin(args: &[*const u8], n: usize) {
    if n < 3 { println("ilogin: usage: ilogin <note> <password>"); return; }
    let note = as_str(args[1]); let pw = as_str(args[2]);
...
fn cmd_ilogout(_: &[*const u8], _: usize) { auth_logout(); println("Logged out"); }

fn cmd_iwho(_: &[*const u8], _: usize) {
    let pwid = proc_get_pwid();
    if pwid == 0 { println("Not logged in"); } else { print("PWID: "); print_hex(pwid); print("\n"); }
}

fn cmd_ipasswd(_: &[*const u8], _: usize) {
...
fn cmd_shost(args: &[*const u8], n: usize) {
    if n == 1 {
        let mut buf = [0u8; 64];
        if gethostname(&mut buf) == 0 { println(core::str::from_utf8(&buf).unwrap_or("?").trim_end_matches('\0')); }
        else { println("Error"); }
    } else {
...
fn cmd_sver(_: &[*const u8], _: usize) {
...
static BUILTINS: &[Builtin] = &[
    Builtin { name: "help",    func: cmd_help },    Builtin { name: "cls",     func: cmd_cls },
    Builtin { name: "echo",    func: cmd_echo },    Builtin { name: "exit",    func: cmd_exit },
    Builtin { name: "fls",     func: cmd_fls },     Builtin { name: "fcd",     func: cmd_fcd },
    Builtin { name: "fpwd",    func: cmd_fpwd },    Builtin { name: "fcat",    func: cmd_fcat },
    Builtin { name: "fmk",     func: cmd_fmk },     Builtin { name: "fmd",     func: cmd_fmd },
    Builtin { name: "frm",     func: cmd_frm },     Builtin { name: "fput",    func: cmd_fput },
    Builtin { name: "fsync",   func: cmd_fsync },   Builtin { name: "ilogin",  func: cmd_ilogin },
    Builtin { name: "ilogout", func: cmd_ilogout }, Builtin { name: "iwho",    func: cmd_iwho },
    Builtin { name: "ipasswd", func: cmd_ipasswd }, Builtin { name: "shost",   func: cmd_shost },
...
fn execute(args: &[*const u8], argc: usize) {
    ...
    for b in BUILTINS { if cmd == b.name { (b.func)(args, argc); return; } }
