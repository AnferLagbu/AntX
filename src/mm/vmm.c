...
static pte_t* get_page_table(uint64_t table, uint64_t index, int create) {
    ...
    if (entry->fields.present) {
        if (entry->fields.pat) {
            ...
            for (int i = 0; i < 512; i++) {
                pte_t* new_entry = &new_table_virt[i];
                new_entry->value = 0;
        ...
        return (pte_t*)PHYS_TO_VIRT((uint64_t)(entry->fields.frame << 12));
    }
...
void vmm_map_page(uint64_t virt, uint64_t phys, uint64_t flags) {
    pte_t* pdpt = get_page_table(kernel_pml4, PML4_INDEX(virt), 1);
    if (pdpt == NULL) return;
    
    pte_t* pd = get_page_table(VIRT_TO_PHYS((uint64_t)pdpt), PDPT_INDEX(virt), 1);
    if (pd == NULL) return;
    
    pte_t* pt = get_page_table(VIRT_TO_PHYS((uint64_t)pd), PD_INDEX(virt), 1);
    if (pt == NULL) return;
...
void vmm_unmap_page(uint64_t virt) {
    pte_t* pdpt = get_page_table(kernel_pml4, PML4_INDEX(virt), 0);
    if (pdpt == NULL) return;
    
    pte_t* pd = get_page_table(VIRT_TO_PHYS((uint64_t)pdpt), PDPT_INDEX(virt), 0);
    if (pd == NULL) return;
    
    pte_t* pt = get_page_table(VIRT_TO_PHYS((uint64_t)pd), PD_INDEX(virt), 0);
    if (pt == NULL) return;
...
uint64_t vmm_get_physical_in_table(uint64_t pml4, uint64_t virt) {
    pte_t* pdpt = get_page_table(pml4, PML4_INDEX(virt), 0);
    if (pdpt == NULL) return 0;
...
static void set_user_bit_recursive(uint64_t table, int level) {
    pte_t* entries = (pte_t*)PHYS_TO_VIRT(table);
    for (int i = 0; i < 512; i++) {
...
uint64_t vmm_create_user_page_table(void) {
    ...
    for (int i = 256; i < 512; i++) {
        new_pml4[i].value = ((pte_t*)PHYS_TO_VIRT(kernel_pml4))[i].value;
    }
    
    for (int i = 0; i < 256; i++) {
        if (((pte_t*)PHYS_TO_VIRT(kernel_pml4))[i].fields.present) {
            new_pml4[i].value = ((pte_t*)PHYS_TO_VIRT(kernel_pml4))[i].value;
        }
    }
...
void vmm_map_page_in_table(uint64_t pml4, uint64_t virt, uint64_t phys, uint64_t flags) {
    pte_t* pdpt = get_page_table(pml4, PML4_INDEX(virt), 1);
    if (pdpt == NULL) return;
...
void vmm_destroy_page_table(uint64_t pml4) {
    pte_t* pml4_ptr = (pte_t*)PHYS_TO_VIRT(pml4);
    
    // Only free user-space page tables (indices 0-255)
    // Kernel-space (indices 256-511) are shared with kernel and should not be freed
    for (int i = 0; i < 256; i++) {
