...
impl KmemCache {
    ...
    pub fn allocate(&mut self) -> Option<*mut u8> {
        ...
        if !self.slabs_free.is_null() {
            self.cache_hits += 1;
            return self.alloc_from_slab(self.slabs_free);
        }
    ...
    fn destroy_slab(&self, slab: *mut SlabHeader) {
        ...
        unsafe {
            ...
            let layout = core::alloc::Layout::from_size_align(SLAB_DEFAULT_SIZE, 4098).unwrap();
    ...
    fn find_free_bit(&self, slab: *mut SlabHeader) -> Option<u32> {
        unsafe {
            let header = &*slab;
            let bitmap_bytes = (header.obj_count + 7) / 8;
    ...
    /// 从双向链表中移除节点
    fn list_remove(&mut self, head: &mut *mut SlabHeader, slab: *mut SlabHeader) {
    ...
    /// 将节点插入到双向链表头部
    fn list_push_front(&mut self, head: &mut *mut SlabHeader, slab: *mut SlabHeader) {
        if slab.is_null() {
            return;
        }
...
pub extern "C" fn slab_system_init() -> i32 {
    ...
    static INIT_MSG: &[u8] = b"Initializing Slab allocator...\0";
    unsafe {
        klog_write(LogLevel::Info as u8, LogCategory::Memory as u8,
    ...
    static OK_MSG: &[u8] = b"Slab system initialized with 8 general caches\0";
    unsafe {
        klog_write(LogLevel::Info as u8, LogCategory::Memory as u8,
                  core::ptr::null(), core::ptr::null(), 0,
...
pub extern "C" fn slab_dump_all_caches() {
    ...
    static HEADER: &[u8] = b"=== Slab Allocator Status ===\0";
    unsafe {
        klog_write(LogLevel::Info as u8, LogCategory::Memory as u8,
                  core::ptr::null(), core::ptr::null(), 0,
